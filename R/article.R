...
#'
#'   AGU recommends `\cite{key}` for parenthetical citations and
#'   `\citeA{key}` for textual citations. In these commands, use angle brackets
#'   for prenotes and square brackets for postnotes, for example
#'   `\cite<e.g.,>[p. 4]{key}`. The default `citation_package = "natbib"` is
#'   retained for compatibility with Markdown citations generated by Pandoc.
#'
#'   New drafts include `agujournal2019.cls`. Existing drafts that retain only
#'   a project-local `agujournal2018.cls` automatically use that legacy class
#'   with the unchanged `rticles::agu_article` output declaration. If both
#'   classes are present, the current 2019 class is used.
#' @export
#' @rdname article
