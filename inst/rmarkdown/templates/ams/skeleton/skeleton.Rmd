...
author2: 
  name: Author Two 
  affiliation: "Current affiliation: NCAR, Boulder, Colorado."
affiliation: "American Meteorological Society,Boston, Massachusetts"
extra_authors: 
  - name: Extra Author
    affiliation: Affiliation, City, State/Province, Country 
abstract: |
...
The text should be divided into sections, each with a separate heading and consecutive numbering. Note, however, that single secondary, tertiary, andquaternary sections remain unnumbered. Each section heading should be placed on a separate line using the appropriate \LaTeX\ commands. 

## Secondary heading {-}

Secondary headings labeled with letters are formatted using `## Heading {-}` for a single subsection within a section or `## Heading` for multiple subsections within one section.

### Tertiary headings {-}

Tertiary headings are formatted using `### Heading {-}` for single a subsubsection within a subsection or `### Heading` for multiple subsubsections within a subsection. 

#### Quaternary headings {-}

Quaternary headings are formatted using `#### Heading {-}` for a single paragraph within a subsubsection or `#### Heading` for multiple paragraphs within a subsection.

# Citations

Citations to standard references in text should consist of the name of the author and the year of publication, for example, [@poe12] or @poe12 using `[@poe12]` or `@poe12`, respectively. A variety of citation formats can be used with the natbib package; however, the AMS prefers that authors use only these commands. References should be entered in the references.bib file. For a thorough discussion of how to enter references into the references.bib database file following AMS style, please refer to the \textbf{AMS\_RefsV5.pdf} document included in the sample documents available here: \url{https://www.ametsoc.org/ams/index.cfm/publications/authors/journal-and-bams-authors/latex-author-info/}
...
The AMS prefers that all figures and tables are placed **at the end of the document** prior to submission. A list of tables and a list of figures will appear near the end of the PDF file, before the actual tables and figures. These lists are necessary for submission.
...
The insertion of a sample figure (Fig. \ref{fig:f1}) and caption is given at the end of the document. Standard figure sizes are 19 (one column), 27, 33, and 39 (two columns) picas.


## Tables

Each table must be numbered, provided with a caption, and mentioned specifically in the text.  See at the end of the document for the formatting of a sample table (Table \ref{t1}).
...
The title of the appendix can be formatted using the `\appendixtitle{}` command. `## Subsection`, `### Subsubection`, and `#### Paragraph` are used to create sections within the appendix. (Note that the appendix title takes the place of `# Section`  in the appendix, so the first section should begin with `##` instead of `#`.) Equations are automatically numbered appropriately for  each appendix. Here is an example of the first equation in appendix A, automatically labeled (\ref{eq:1}): 
...
For figures, this .Rmd template adds a chunk to use the correct caption command. See below. 
...
The AMS uses the Editorial Manager system for all author submissions for peer review. Editorial Manager uses the freely available \TeX\ Live 2018 distribution. This system will automatically generate a PDF from your submitted \LaTeX\ files and figures.  
...
```{r f1, fig.cap="This is a sample figure with caption"}
plot(pressure)
```

```{r A1, fig.cap="Here is the apendix figure caption", appendix = TRUE}
plot(pressure)
```
