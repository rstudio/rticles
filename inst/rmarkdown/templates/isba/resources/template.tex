...
$if(author.value.email)$\ead[label=ea-$author.key$,email]{$author.value.email$}$endif$
$if(author.value.url)$\ead[label=ua-$author.key$,url]{$author.value.url$}$endif$}$sep$, $endfor$
