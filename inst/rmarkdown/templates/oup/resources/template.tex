...
\newcommand*\corresplblbycode[1]{%
  \ifcounter{CORRESPLBL@#1}
    {}
    {\refstepcounter{correspcnt}\newcounter{CORRESPLBL@#1}\setcounter{CORRESPLBL@#1}{\value{corresp}}}%
...
$endif$
$if(address.id)$
\MyNewLabel{ADDRTXT}{$address.id$}{$address.address$}
$endif$
$endfor$
$endif$
...
$for(footnote)$
$if(footnote.code)$
\MyNewLabel{FTNTXT}{$footnote.code$}{$footnote.text$}
$endif$
$endfor$
$endif$
...
$for(author/first)$\authormark{$it.name$ et al.}$endfor$
