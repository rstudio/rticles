...
journals, see Table \ref{tab:mdpinames} to find the appropriate short journal name.


\startlandscape
```{r, mdpinames, echo=FALSE, results='asis'}
## takes a character string of journal names and turns them into a table
journals_to_table <- function(x, n) {
  journals <- unlist(strsplit(x, ", "))
  journals <- split(journals, cut(seq_along(journals),n))
  max_n <- max(unlist(lapply(journals, length)))
  for (i in 1:n) {
    length(journals[[i]]) <- max_n
  }
  journals <- data.frame(journals)
  names(journals) <- 1:n
  journals
}
...
mdpi_journals <- journals_to_table(mdpi_journals, 8)
knitr::kable(mdpi_journals,
             format = "latex",
             booktabs = TRUE, 
