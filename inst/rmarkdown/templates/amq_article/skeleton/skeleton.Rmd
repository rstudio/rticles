...
- name: Joseph Bleau
  ...
  email: \href{mailto:jo-bleau@cegepfleurimont.qc.ca}{contact par courriel avec l'auteur}
...
Le résumé apparaît ici. Long d'au maximum 100 mots, il peut contenir des expressions en  {\em italique}, en {\bf caractàres gras}, entre \og guillemets\fg~ou en \textsc {petites capitales}. On peut aussi utiliser des symboles particuliers comme $\Bbb{R}$, $\mathscr{F}$ ou encore $\mathscr{P}(E)$ et même des expressions mathématiques comme $f_k(x)=a x^2+bx+c$, ou encore à  l'ensemble $\mathfrak{F}$ des formules du système formel $\mathfrak{L}$ est bien défini?.
...
Les sections et sous-sections sont numérotées de façon automatique. La même rêgle s'applique, catégorie par catégorie, aux théorèmes, aux lemmes, aux corollaires et aux définitions.
