---
title: "Tuftish e-Book"
author: "John Smith"
date: "February 8, 2015"
bibliography: "knitcitations.bib"
output: 
  rticles::tuftish_ebook:
...
This skeleton presents an overview of the key Tuftish-book capabilities, and examples of some of R packages that can be used with the template for pretty printing.  

# Known Bug

If you choose LuaTeX as your TeX engine, character spacing will be wrong on the title page and page headers.  This is related to a known-bug in a LaTeX package that has existed for at least four years.  Please don't report this to Rstudio or on the git as a bug.  Feel free to discuss it here, though: https://code.google.com/p/tufte-latex/issues/detail?id=45.   And do feel free to make a pull request if you have a workaround. 

For most uses, pdfTeX is fine anyway, and XeTeX better.  If tikZ is running out of memory, then consider switching to LuaTeX.  
...
Some people feel these are unnecessary. 
...
```{r, results='asis'}
xtable(head(mtcars[,1:6]), caption = "xtable, first rows of mtcars")
```
...
pander(head(mtcars[,1:5]),caption="pander, first rows of mtcars")
```
...
```{r kable}
kable(head(mtcars[,1:5]), caption = "kable, first rows of mtcars")
```
