...
**How to Make a Side Bar**
  ...
  **Use a [custom block](https://bookdown.org/yihui/bookdown/custom-blocks.html) of type 'sidebar'**. Sidebars need to be placed in the `sidebar` \LaTeX\ environment. To achieve that, use the syntax ```` ```{block, type='sidebar'} ```` to begin such a block, ending the block with another three backticks.
  
  Use this approach for arbitrary text you want to put over her - for dynamically created tables and figures, there is a separate syntax, as will be explained later.
  
  - If you want to add a caption for this kind of sidebar, you have to create this with \LaTeX\ syntax directly, e.g.  `\caption{my caption}`.
  - Similarly, if you want a label for this kind of sidebar so you can refer to it, you must add with \LaTeX\ syntax within the sidebar, e.g. `\label{bar:arbitrary-sidebar}`.
