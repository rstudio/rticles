...
abstract: This is a guided illustration of how to write a CHI Extended Abstracts paper in R Markdown, using the latest official ACM \LaTeX\ template [Version 1.55, September 11, 2018](https://www.acm.org/publications/proceedings-template). This R package is last updated on 23rd October 2018. #If you put the abstract in quotes, you must escape LaTeX syntax with an extra '\', e.g. '\\LaTeX\\'.
keywords: CHI Extended Abstracts; \LaTeX; R Markdown; reproducible papers
bibliography: sample-bibliography
output: 
  rticles::acm_chi_ea_article:
    latex_engine: xelatex #if you use pdflatex, some special characters like ẽ might cause error
...
\begin{sidebar}
  ...
  \textbf{Use \LaTeX\ Syntax:} Sidebars need to be placed in the \texttt{sidebar} \LaTeX\ environment. To achieve that, you can either: 1) write the text for the sidebar with markdown syntax, compile to PDF (if you are using RStudio, then just click the 'Knit' button), then open the generated \textbf{.tex} file for your manuscript wrap the desired parts inside \texttt{{\textbackslash}begin\{sidebar\}} and \texttt{{\textbackslash}end\{sidebar\}}, 2) simply all of the content for your sidebar using \LaTeX\ syntax in your R Markdown manuscript.
  ...
  In the source file for this example document, the sidebar is written in \LaTeX\ syntax inside the R Markdown document.
  
  \textbf{Margin figures and tables:} For these, however, there is R Markdown syntax you can use to automatically put dynamically generated tables or figures, as well as static figures, in the right \LaTeX\ environment. This is explained later.
...
# Paper meta data
Set meta data (copyright, authors, keywords, title, keywords, etc.) in the YAML header of the .Rmd file in which you write the manuscript. This is done in the form of `key: value` pairs, e.g. `title: Writing CHI Proceedings Papers With R Markdown`. When compiling to a PDF (in RStudio, just click the 'Knit' button), the information in the YAML header is plugged into the CHI Extended Abstracts \LaTeX\ template (specified in the YAML header with `template: sample-sigchi-a.tex`). If you take a look at this template file, you will see e.g. `\title[$short-title$]{$title$}` - things between dollar signs are interpreted as a variable to be searched for in the YAML header and plugged into the template when generating a PDF.

**Note the sole exception for adding paper meta data**: The CCS Concepts are messy to insert from the YAML header, so you should manually insert this into either the template `sample-sigchi-a.tex` or the **.tex** file generated for your manuscript when you knit to PDF.
...
# by default, don't include code output, messages, or warnings in manuscript 
```
````
```{r setup, include=FALSE}
...
# Finishing touches
For the Extended Abstracts template in particular, it can be fiddly to get the final touches of the layout right, mostly in relation to the arrangement of the tables and figures in the margin. By default, `keep_tex:` is therefore set to `true` in the YAML header. This keeps the intermediary **.tex** file in your working directory after you have compiled to PDF. You can then make finishing touches in this file (e.g. using [OverLeaf](https://www.overleaf.com)), e.g. you might want to play around with varying amounts of `\vspace` to position the margin content.
